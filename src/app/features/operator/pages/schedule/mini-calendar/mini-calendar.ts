...
interface CalendarDay {
  ...
  isCurrentMonth: boolean; // false = dia "relleno" de otro mes
  serviceCount: number;
  isToday: boolean;
}
...
export class MiniCalendarComponent implements OnChanges {
  ...
  get monthTitle(): string {
    ...
    const name = months[this.displayedMonth.getMonth()];
    return `${name} ${this.displayedMonth.getFullYear()}`;
  }
  ...
  private generateCalendar() {
    ...
    for (let i = 0; i < totalCells; i++) {
      ...
      const dateText = this.dateToText(date);
